...
def remove_info(text, journal_id, label, doc_type='inkomst'):
    ...
    if doc_type == 'epikris':
        ...
        if journal_id == '6358467':
            cleaned_text = re.sub('Diagnose : .* (?=.)', '', cleaned_text)  # handles 6358467 (Pre_R55) at the diagnose end
        #if 'Diagnose :' not in text:
        #    print('no D: {} ({})'.format(journal_id, label))
        if 'Diagnose' in cleaned_text and journal_id != '121101179': # no info after diagnosis (TO DO: handle better)
            print('Diagnose left: {} ({})'.format(journal_id, label))
        if 'Problemstilling' in cleaned_text and journal_id not in ['38367491', '7361052']:
            print('Problemstilling left: {} ({})'.format(journal_id, label))
    return cleaned_text
