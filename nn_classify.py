...
def build_model(model_type, vocab_size, max_length, embedding_dims, embedding_weights, trainable):
    ...
    if model_type == 'joulin': 
        ...
        filters = 128   #64    
        kernel_size = 5 #3
        model = nn_models.cnn(vocab_size, embedding_dims, embedding_weights, 
                              trainable, filters, kernel_size)
    elif model_type == 'cnn_jacovi':
        filters = 256   #64, 128, 256  
        kernel_size = 2 #2-5
        model = nn_models.cnn_jacovi(vocab_size, embedding_dims, embedding_weights, 
                                     trainable, filters, kernel_size)
        ...
        raise NameError('''No such model. Choose between:
                        "joulin", "cnn", "cnn_jacovi", "cnn_kim" or "lstm"''')
    return model 
